        ...
        <>
            <div className="table__header__">
                ...
                <div className="pagination">
                    <span>Page</span>
                    <select name="page" id="page" value={currentPage}
                        onChange={(e) => setCurrentPage(Number(e.target.value))}>
                        {[...Array(totalPages).keys()].map((page) => (
                            <option key={page + 1} value={page + 1}>{page + 1}</option>
                        ))}
                    </select>
                    <button onClick={handlePreviousPage} disabled={currentPage === 1} className='btn-line'><IoIosArrowRoundBack /></button>
                    <button onClick={handleNextPage} disabled={currentPage === totalPages}><IoIosArrowRoundForward /></button>
                </div>
            </div>
